...
> **Meta-rule:** every rule in this document must be machine-enforceable. Rules that cannot yet be enforced mechanically are marked _(deferred)_ and excluded from the toolchain until a concrete gate can be defined.
...
| `wires`      | construct concrete classes     | calling their methods directly          | _(deferred)_              |
...
| `uses`       | call pure functions            | anything stateful                       | _(deferred)_              |
